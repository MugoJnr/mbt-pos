...
echo  [2e/5] Building web Command Center (npm)...
...
if exist "dist\MBT_POS_Setup.exe" (
    ...
    copy /Y "dist\MBT_POS_Setup.exe" "!DEST1!"
    if errorlevel 1 (
        echo  [!] Could not copy to OneDrive Desktop: !DEST1!
    ) else (
        echo  [OK] Copied to !DEST1!
    )
    copy /Y "dist\MBT_POS_Setup.exe" "!DEST2!" >nul 2>&1
    if exist "!DEST2!" echo  [OK] Copied to !DEST2!
    if exist "dist\MBT_POS_Setup.exe.sha256" (
        echo  SHA-256 sidecar:
        type "dist\MBT_POS_Setup.exe.sha256"
    )
)
echo.
